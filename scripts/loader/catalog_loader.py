...
def build_sql_from_json(json_text: str, *, source_file: str) -> str:
    """Insert-if-new into catalog_reports_json with the finished catalog JSON payload.

    Uses a unique constraint (uq_catalog_json) on (report_code, report_date)
    enforced at the JSON landing table. If a matching row already exists,
    skip it (no update).
    """
...
INSERT INTO beef_data.catalog_reports_json (source_file, payload)
SELECT {source_file!r}, j FROM doc
ON CONFLICT ON CONSTRAINT uq_catalog_json DO NOTHING;
COMMIT;
"""
...
def run() -> None:
    ...
    in_dir = processed_base / "processed_catalog" / "finished_catalog"
    ...
    if needs_full_reload:
        print("Catalog table history is incomplete; reloading full history from processed files.")
        reset_sql = (
            "SET search_path TO beef_data, public;\n"
            "TRUNCATE TABLE beef_data.catalog_reports_json;\n"
        )
        subprocess.run(base_args, input=reset_sql.encode("utf-8"), check=True)
    else:
        print(f"Catalog history already spans {db_first_date} through {db_last_date}; checking for updates only.")
        files = [p for p in files if (extract_date_from_name(p.name) or date.min) > db_last_date]
        if not files:
            print("Catalog data already up to date.")
            return
