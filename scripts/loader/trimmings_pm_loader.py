...
def build_sql_from_json(json_text: str, *, source_file: str) -> str:
    """Insert-if-new into trimmings_pm_reports_json with the processed PM JSON payload.

    Unique constraint `uq_trimmings_pm_json` ensures one row per
...
INSERT INTO beef_data.trimmings_pm_reports_json (source_file, payload)
SELECT {source_file!r}, j FROM doc
ON CONFLICT ON CONSTRAINT uq_trimmings_pm_json DO NOTHING;
COMMIT;
"""
...
def run() -> None:
    ...
    in_dir = processed_base / "processed_trimmings_pm"
    ...
    last_date = read_last_date("trimmings_pm")
    if last_date:
        print(f"Resuming trimmings PM from > {last_date}")
        files = [p for p in files if (extract_date_from_name(p.name) or date.min) > last_date]
    else:
        print("No previous trimmings PM state; full load")
    ...
    for path in files:
        ...
        if d is not None:
            write_last_date("trimmings_pm", d)
    print("Trimmings PM load completed.")
