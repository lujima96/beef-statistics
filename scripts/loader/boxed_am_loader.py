...
def build_sql_from_json(json_text: str, *, source_file: str) -> str:
    """Insert-if-new into simplified JSON landing table.

    The new schema stores AM boxed beef reports as a single JSONB payload in
    `beef_data.boxed_am_reports_json`. Convenience columns (report_id, date,
    run, etc.) are generated from the payload and a unique constraint
    `uq_boxed_am` enforces one row per report_id/date/run. If a matching row
    already exists, skip it (no update).
    """
    b64 = base64.b64encode(json_text.encode("utf-8")).decode("ascii")
    # Use base64 to safely pass JSON via psql stdin; decode back to jsonb in SQL
    return f"""
...
INSERT INTO beef_data.boxed_am_reports_json (source_file, payload)
SELECT {json.dumps(source_file)!r}, j FROM doc
ON CONFLICT ON CONSTRAINT uq_boxed_am DO UPDATE
...
def is_minimal_boxed_payload(s: str) -> bool:
    try:
        obj = json.loads(s)
    except Exception:
        return False
    return isinstance(obj, dict) and "sub_primal_negotiated_cut_prices" in obj
...
def run() -> None:
    ...
    try:
        # Try importing config for canonical dirs
    ...
    in_dir = processed_base / "processed_boxed_am"
    ...
    if needs_full_reload:
        print("Boxed AM table history is incomplete; reloading full history from processed files.")
        reset_sql = (
            "SET search_path TO beef_data, public;\n"
            "TRUNCATE TABLE beef_data.boxed_am_reports_json;\n"
        )
        subprocess.run(base_args, input=reset_sql.encode("utf-8"), check=True)
    else:
        print(f"Boxed AM history already spans {db_first_date} through {db_last_date}; checking for updates only.")
        files = [p for p in files if (extract_date_from_name(p.name) or date.min) > db_last_date]
        if not files:
            print("Boxed AM data already up to date.")
            return

    for path in files:
        text = path.read_text(encoding="utf-8")
        if not is_minimal_boxed_payload(text):
            print(f"Skipping {path.name}: payload missing sub_primal_negotiated_cut_prices")
            continue
        ...
        if d is not None:
            write_last_date("boxed_am", d)
    print("Boxed AM load completed.")
