...
def build_sql_from_json(json_text: str, *, source_file: str) -> str:
    """Insert-if-new into boxed_pm_reports_json with the JSON payload.

    Mirrors the AM loader but targets the PM landing table. A unique
    constraint `uq_boxed_pm` ensures one row per (report_id, report_date, run).
...
INSERT INTO beef_data.boxed_pm_reports_json (source_file, payload)
SELECT {source_file!r}, j FROM doc
ON CONFLICT ON CONSTRAINT uq_boxed_pm DO UPDATE
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                FROM beef_data.boxed_pm_reports_json
                """
            )
            row = cur.fetchone()
    if not row:
...
def run() -> None:
    ...
    in_dir = processed_base / "processed_boxed_pm"
    ...
    if needs_full_reload:
        print("Boxed PM table history is incomplete; reloading full history from processed files.")
        reset_sql = (
            "SET search_path TO beef_data, public;\n"
            "TRUNCATE TABLE beef_data.boxed_pm_reports_json;\n"
        )
        subprocess.run(base_args, input=reset_sql.encode("utf-8"), check=True)
    else:
        print(f"Boxed PM history already spans {db_first_date} through {db_last_date}; checking for updates only.")
        files = [p for p in files if (extract_date_from_name(p.name) or date.min) > db_last_date]
        if not files:
            print("Boxed PM data already up to date.")
            return
    ...
    for path in files:
        ...
        if d is not None:
            write_last_date("boxed_pm", d)
    print("Boxed PM load completed.")
