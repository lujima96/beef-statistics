#!/usr/bin/env python3
"""Update the National Daily Cattle & Beef Summary link list from MyMarketNews."""
...
PAGE_TIMEOUT_SEC = 20
PAGE_SETTLE_SEC = 1.0
TARGET_SUFFIXES = (".pdf", ".txt")
DEFAULT_LINKS_FILE = (
    Path(__file__).resolve().parents[2]
    / "links"
    / "national_daily_cattle_&_beef_summary.txt"
)
...
def _build_page_url(page_index: int) -> str:
    return _build_page_url_for_base(PRIMARY_BASE_URL, page_index)


def _build_page_url_for_base(base_url: str, page_index: int) -> str:
    if page_index <= 0:
        return base_url
...
def _parse_current_page(driver: webdriver.Chrome) -> List[Tuple[dt.date, str]]:
    ...
    for row in rows:
        try:
            ...
            href = urljoin(PRIMARY_BASE_URL, link_el.get_attribute("href") or "")
...
def main(links_file: Path = DEFAULT_LINKS_FILE) -> None:
    ...
    page_index = 0
    active_base_url = PRIMARY_BASE_URL
    ...
    try:
        while True:
            ...
            page_url = _build_page_url_for_base(active_base_url, page_index)
            ...
            if not page_links:
                if page_index == 0 and not fallback_checked and active_base_url != FALLBACK_BASE_URL:
                    print("No rows on slug-filtered page; retrying with report-code search.")
