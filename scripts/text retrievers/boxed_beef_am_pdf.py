"""PDF-to-text conversion helpers for the Boxed Beef AM retriever."""
...
def pdf_bytes_to_text(pdf_bytes: bytes) -> str:
    """Convert raw PDF bytes into normalized text."""

    if HAS_FITZ:
        try:  # pragma: no branch - successful path exits early
    ...
    try:
        ...
        fallback_text = pdfminer_extract_text(BytesIO(pdf_bytes), laparams=laparams) or ""
        return _normalize_text(fallback_text)
    except Exception:
        fallback_text = pdfminer_extract_text(BytesIO(pdf_bytes)) or ""
        return _normalize_text(fallback_text)
