"""HTTP utilities for fetching Boxed Beef AM report content."""
...
from boxed_beef_am_config import BACKOFF_BASE_SEC, MAX_RETRIES, TIMEOUT
...
def _browser_fetch_response(url: str) -> requests.Response:
    driver = _get_browser()
    driver.get(f"https://{MYMARKETNEWS_HOST}/filerepo/reports?field_slug_id_value=2452")
...
def fetch_response(url: str) -> requests.Response:
    """Fetch ``url`` with retries and a polite user agent."""
...
def sniff_is_pdf(resp: requests.Response, sample: bytes) -> bool:
    """Return ``True`` if the response payload looks like PDF data."""
