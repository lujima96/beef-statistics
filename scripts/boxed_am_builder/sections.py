"""Helpers for parsing tabulated sections of boxed AM reports."""
...
def parse_subprimal_section(section: str) -> list[dict[str, Any]]:
    """Parse tabulated sub-primal data from a report section."""
...
def parse_simple_section(section: str) -> list[dict[str, Any]]:
    """Parse sections with a simpler item + numeric tail format."""
    ...
    for line in section.splitlines():
        ...
        if match:
            ...
            fallback = re.search(r"(\d+)\s+([\d,]+)\s*$", line)
            if fallback:
                name = line[: fallback.start()].strip()
                trades = int(fallback.group(1))
                pounds = int(fallback.group(2).replace(",", ""))
                low = high = avg = None
            else:
