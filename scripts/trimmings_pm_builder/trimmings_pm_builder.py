...
RAW_TRIMMINGS_PM_DIR = RAW_DIR / "raw_trimmings_pm"
PROCESSED_TRIMMINGS_PM_DIR = PROCESSED_DIR / "processed_trimmings_pm"
...
def parse_trimmings_pm(text: str) -> dict:
    """Parse raw trimmings PM text into structured JSON."""
    # Report code
...
def run() -> None:
    os.makedirs(PROCESSED_TRIMMINGS_PM_DIR, exist_ok=True)
    for file in sorted(RAW_TRIMMINGS_PM_DIR.glob("*.txt")):
        date_str = file.stem.split("_")[0]
        out_path = PROCESSED_TRIMMINGS_PM_DIR / f"{date_str}.json"
        if out_path.exists():
            continue
        text = file.read_text(encoding="utf-8", errors="ignore")
        parsed = parse_trimmings_pm(text)
