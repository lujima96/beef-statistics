import { Point, Range, View } from './Chart5DayAvgTypes'
...
export function interpolateInterior(points: { date: string; value: number | null }[]) {
...
// Remove improbable spikes using neighbor comparison.
export function despike(points: { date: string; value: number | null }[]) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const diff = Math.abs((v as number) - expected)
