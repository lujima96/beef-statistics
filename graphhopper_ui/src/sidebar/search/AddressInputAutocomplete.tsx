...
export class VendorItem implements AutocompleteItem {
    id: number
    name: string
    address: string
    ...
    constructor(id: number, name: string, address: string, point: { lat: number; lng: number }) {
        this.id = id
        this.name = name
        this.address = address
        this.point = point
        this.searchText = [name, address].join(' ').toLowerCase()
    }

    matches(query: string) {
        return !query || this.searchText.includes(query)
    }
...
function mapToComponent(item: AutocompleteItem, isHighlighted: boolean, onSelect: (hit: AutocompleteItem) => void) {
    ...
    else if (item instanceof VendorItem) return <VendorEntry item={item} isHighlighted={isHighlighted} onSelect={onSelect} />
...
function VendorEntry({
    ...
    return (
        <AutocompleteEntry isHighlighted={isHighlighted} onSelect={() => onSelect(item)}>
            <div className={styles.vendorEntry}>
                <span className={styles.vendorName}>{item.toLabel()}</span>
                {item.address && <span className={styles.vendorAddress}>{item.address}</span>}
            </div>
        </AutocompleteEntry>
    )
}
