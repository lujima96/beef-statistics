...
export default function AddressInput(props: AddressInputProps) {
    ...
    const vendorMatcherRef = useRef<(query: string) => VendorItem[]>(() => [])
    const geocodeItemsRef = useRef<AutocompleteItem[]>([])

    const vendorItems = useMemo(() => {
        return props.vendorOptions
            .filter(
                vendor =>
                    vendor &&
            ...
            .map(
                vendor =>
                    new VendorItem(vendor.id, vendor.name || `Vendor ${vendor.id}`, vendor.address || '', {
                        lat: vendor.latitude,
    ...
    const filterVendorItems = useCallback(
        (query: string) => {
            const normalized = (query || '').trim().toLowerCase()
            if (!normalized) return vendorItems
            return vendorItems.filter(item => item.matches(normalized))
        },
        [vendorItems],
    )

    vendorMatcherRef.current = filterVendorItems

    const [geocoder] = useState(
        () =>
            new Geocoder(getApi(), (query, provider, hits) => {
                const geocodeItems: AutocompleteItem[] = []
                const parseResult = AddressParseResult.parse(query, true)
                if (parseResult.hasPOIs()) geocodeItems.push(new POIQueryItem(parseResult))
                ...
                geocodeItemsRef.current = geocodeItems
                const combined = [...vendorMatcherRef.current(query), ...geocodeItems]
                setOrigText(query)
                setAutocompleteItems(combined)
                setOrigAutocompleteItems(combined)
            }),
    )
            ...
            switch (event.key) {
                    ...
                    setHighlightedResult(i => {
                        ...
                        if (autocompleteItems.length > 0) {
                            if (nextIndex < 0) {
                                ...
                                else if (item instanceof VendorItem) setText(item.toLabel())
                ...
                case 'Tab':
                    ...
                    if (coordinate) {
                        ...
                        } else if (item instanceof VendorItem) {
                            props.onAddressSelected(item.toLabel(), item.point)
                            setOrigText(item.toLabel())
            ...
            >
                ...
                <input
                    ...
                    onChange={e => {
                        ...
                        setText(query)
                        const vendorMatches = filterVendorItems(query)
                        geocodeItemsRef.current = []
                        setAutocompleteItems(vendorMatches)
                        setOrigAutocompleteItems(vendorMatches)
                    ...
                    onFocus={() => {
                        ...
                        if (origAutocompleteItems.length > 0) setAutocompleteItems(origAutocompleteItems)
                            ...
                            const vendorMatches = filterVendorItems(text)
                            setAutocompleteItems(vendorMatches)
                            setOrigAutocompleteItems(vendorMatches)
                        }
                    }}
                        ...
                        <Autocomplete
                            ...
                            onSelect={item => {
                                if (item instanceof GeocodingItem) {
                                    props.onAddressSelected(item.toText(), item.point)
                                    setOrigText(item.mainText)
                                } else if (item instanceof VendorItem) {
                                    props.onAddressSelected(item.toLabel(), item.point)
                                    setText(item.toLabel())
                                    setOrigText(item.toLabel())
...
class Geocoder {
    ...
    async requestAsync(query: string, bias: Coordinate | undefined, zoom: number) {
        ...
        try {
            ...
            if (reason instanceof Error && /geocoding went wrong 40[024]/i.test(reason.message)) {
                console.debug('Geocoder suppressed error for query', query, reason)
                return
            }
