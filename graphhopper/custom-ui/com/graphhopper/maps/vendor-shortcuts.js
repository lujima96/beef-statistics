(function () {
  ...
  function attachSelectToInput(input) {
    ...
    var resetHandler = function () {
      if (input.dataset && input.dataset.beefVendorProgrammatic === '1') {
        return
      }
      selectionByInput[inputId] = CUSTOM_VALUE
      if (!select.disabled) {
  ...
  function applyVendorToInput(input, vendor, select) {
    var inputId = assignInputId(input)
    var coordinates = vendor.latitude.toFixed(6) + ',' + vendor.longitude.toFixed(6)
    var label = formatVendorLabel(vendor)
    if (input.dataset) {
      input.dataset.beefVendorProgrammatic = '1'
    } else {
      input.setAttribute('data-beef-vendor-programmatic', '1')
    }
    input.value = coordinates
    input.setAttribute('title', label)
    selectionByInput[inputId] = String(vendor.id)
    if (select) {
      select.value = String(vendor.id)
    }
    var inputEvent
    try {
    ...
    setTimeout(function () {
      if (input.dataset) {
        delete input.dataset.beefVendorProgrammatic
      } else {
        input.removeAttribute('data-beef-vendor-programmatic')
      }
    }, 0)
  }
