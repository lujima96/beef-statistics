...
function determineVendorStateUrl() {
    if (ghenv.beefStatsApi && ghenv.beefStatsApi.host) {
        return normalizeBaseUrl(ghenv.beefStatsApi.host) + '/delivery-estimator/state';
    }
...
var vendorLocations = [];
var vendorLookupById = {};
var vendorLoading = false;
var vendorLoadErrorMessage = '';

var VENDOR_TOGGLE_DEFAULT_HINT = 'Select a saved vendor location';
var VENDOR_LOADING_HINT = 'Loading saved vendor locations…';
var VENDOR_NONE_HINT = 'No saved vendor locations yet';

function ensureVendorToggleAttributes(div) {
    if (!div || !div.length) {
        return;
    }
    var toggle = div.find('.pointVendorToggle');
    if (!toggle.length) {
        return;
    }
    toggle.attr('aria-expanded', div.hasClass('pointVendorOpen') ? 'true' : 'false');
}

function setVendorToggleHint(div, hint) {
    if (!div || !div.length) {
        return;
    }
    var toggle = div.find('.pointVendorToggle');
    ...
    var message = hint || VENDOR_TOGGLE_DEFAULT_HINT;
    toggle.attr('title', message);
    toggle.attr('aria-label', message);
}

function setVendorOpenState(div, shouldOpen) {
    if (!div || !div.length) {
        return;
    }
    if (shouldOpen) {
        div.addClass('pointVendorOpen');
    } else {
        div.removeClass('pointVendorOpen');
    }
    ensureVendorToggleAttributes(div);
}

function getVendorMenu(div) {
    if (!div || !div.length) {
        return $();
    }
    return div.find('.pointVendorMenu');
}

function updateVendorSelectionClasses(div, menu) {
    if (!div || !div.length) {
        return;
    }
    var targetMenu = menu && menu.length ? menu : getVendorMenu(div);
    var hasSelection = false;
    if (targetMenu.length) {
        var selectedId = targetMenu.attr('data-selected-id');
        hasSelection = selectedId !== undefined && selectedId !== null && String(selectedId).length > 0;
    }
    if (hasSelection) {
        div.addClass('pointVendorHasSelection');
    } else {
        div.removeClass('pointVendorHasSelection');
    }
    ensureVendorToggleAttributes(div);
}

function closeOtherVendorMenus(exceptDiv) {
    $('#locationpoints > div.pointDiv').each(function (index, element) {
        var candidate = $(element);
        if (!exceptDiv || candidate.get(0) !== exceptDiv.get(0)) {
            setVendorOpenState(candidate, false);
        }
    });
}

function focusVendorMenu(div) {
    var menu = getVendorMenu(div);
    if (!menu.length) {
        return;
    }
    var selected = menu.find('.pointVendorOptionSelected').first();
    var target = selected.length ? selected : menu.find('.pointVendorOption').first();
    if (target.length) {
...
function setVendorMenuSelection(div, vendorId) {
    var menu = getVendorMenu(div);
    if (!menu.length) {
        return;
    }
    if (vendorId !== undefined && vendorId !== null && String(vendorId).length > 0) {
        var normalizedId = String(vendorId);
        menu.attr('data-selected-id', normalizedId);
        menu.find('.pointVendorOption').each(function (index, element) {
            var option = $(element);
            var isSelected = option.attr('data-vendor-id') === normalizedId;
            option.toggleClass('pointVendorOptionSelected', isSelected);
            if (isSelected) {
        ...
        menu.removeAttr('data-selected-id');
        menu.find('.pointVendorOption').each(function (index, element) {
            $(element).removeClass('pointVendorOptionSelected').removeAttr('aria-selected');
        });
    }
    updateVendorSelectionClasses(div, menu);
}

function formatVendorLabel(vendor) {
    var parts = [];
    if (vendor.locationName) {
        parts.push(vendor.locationName);
    }
    if (vendor.address) {
        parts.push(vendor.address);
    }
    if (!parts.length) {
        parts.push('Vendor ' + vendor.id);
    }
    return parts.join(' \u2013 ');
}

function coordinatesMatchVendor(point, vendor) {
    if (!point || !vendor) {
        return false;
    }
    ...
    var pointLat = Number(point.lat).toFixed(6);
    var pointLng = Number(point.lng).toFixed(6);
    var vendorLat = Number(vendor.latitude).toFixed(6);
    var vendorLng = Number(vendor.longitude).toFixed(6);
    return pointLat === vendorLat && pointLng === vendorLng;
}

function findVendorByCoordinates(lat, lng) {
    var targetLat = Number(lat).toFixed(6);
    var targetLng = Number(lng).toFixed(6);
    for (var i = 0; i < vendorLocations.length; i++) {
        var vendor = vendorLocations[i];
        if (Number(vendor.latitude).toFixed(6) === targetLat && Number(vendor.longitude).toFixed(6) === targetLng) {
            return vendor;
        }
    }
    return null;
...
function setVendorLocations(rawList) {
    vendorLocations = [];
    vendorLookupById = {};
    if (!rawList) {
        return;
    ...
    for (var i = 0; i < rawList.length; i++) {
        ...
        if (typeof id !== 'number' || !isFinite(id) || typeof lat !== 'number' || !isFinite(lat) || typeof lng !== 'number' || !isFinite(lng)) {
            continue;
        }
        ...
        vendorLocations.push(normalized);
        vendorLookupById[String(id)] = normalized;
    }
    vendorLocations.sort(function (a, b) {
...
function applyVendorSelection(index, vendor) {
    if (!vendor) {
        return;
    }
    var coordinates = vendor.latitude + ',' + vendor.longitude;
    var point = ghRequest.route.getIndex(index);
    if (!point) {
        point = ghRequest.route.set(coordinates, index, true);
    } else {
        point.setCoord(vendor.latitude, vendor.longitude);
    }
    if (point) {
        point.vendorId = vendor.id;
    }
    var div = $('#locationpoints > div.pointDiv').eq(index);
    var input = div.find('.pointInput');
    input.val(point ? point.toString() : coordinates);
    input.attr('title', formatVendorLabel(vendor));
    setVendorMenuSelection(div, vendor.id);
    setVendorOpenState(div, false);
    setVendorToggleHint(div, VENDOR_TOGGLE_DEFAULT_HINT);
    resolveIndex(index);
    routeIfAllResolved();
}

function clearVendorSelection(index) {
    var point = ghRequest.route.getIndex(index);
    if (point) {
        delete point.vendorId;
        ghRequest.route.set('', index);
    }
    var div = $('#locationpoints > div.pointDiv').eq(index);
    var input = div.find('.pointInput');
    input.val('');
    input.removeAttr('title');
    setVendorMenuSelection(div, null);
}

function setupVendorSelect(div, index) {
    if (!div || !div.length) {
        return;
    }
    var menu = getVendorMenu(div);
    ...
    var toggle = div.find('.pointVendorToggle');
    if (toggle.length && menu.attr('id')) {
        toggle.attr('aria-controls', menu.attr('id'));
    }
    menu.off('click.vendorOption');
    menu.off('keydown.vendorMenu');
    menu.empty();

    var appendMessage = function (text) {
        menu.attr('aria-disabled', 'true');
        menu.attr('tabindex', '-1');
        menu.append($('<div>').addClass('pointVendorMenuMessage').text(text));
        setVendorMenuSelection(div, null);
        updateVendorSelectionClasses(div, menu);
        ensureVendorToggleAttributes(div);
    };
    ...
    if (vendorLoading) {
        appendMessage('Loading saved locations…');
        setVendorToggleHint(div, VENDOR_LOADING_HINT);
        return;
    }

    if (vendorLocations.length === 0) {
        var emptyMessage = vendorLoadErrorMessage || VENDOR_NONE_HINT;
        appendMessage(emptyMessage);
        setVendorToggleHint(div, emptyMessage);
        return;
    }

    menu.attr('aria-disabled', 'false');
    ...
    var selectedId = '';
    if (point) {
        var candidate = point.vendorId != null ? vendorLookupById[String(point.vendorId)] : null;
        if (candidate && coordinatesMatchVendor(point, candidate)) {
            ...
            if (typeof point.lat !== 'undefined' && typeof point.lng !== 'undefined') {
                var match = findVendorByCoordinates(point.lat, point.lng);
                if (match) {
                    selectedId = String(match.id);
                    point.vendorId = match.id;
                }
            }
        }
    }

    var list = $('<ul>').addClass('pointVendorOptions').attr('role', 'presentation');
    var clearButton = $('<button>')
        .addClass('pointVendorOption pointVendorOptionClear')
        .attr('type', 'button')
        .attr('role', 'option')
        .attr('data-vendor-id', '')
        .text('Enter a custom location');
    clearButton.on('click.vendorOption', function (event) {
        event.preventDefault();
        event.stopPropagation();
        var button = $(this);
        var container = button.closest('.pointDiv');
        var currentIndex = container.data('index');
        clearVendorSelection(currentIndex);
        setVendorOpenState(container, false);
        container.data('suppressVendorFocusOpen', true);
        container.find('.pointInput').trigger('focus');
    });
    list.append($('<li>').append(clearButton));
    for (var i = 0; i < vendorLocations.length; i++) {
        var vendorOption = vendorLocations[i];
        var optionButton = $('<button>')
            .addClass('pointVendorOption')
            .attr('type', 'button')
            .attr('role', 'option')
            .attr('data-vendor-id', String(vendorOption.id))
            .text(formatVendorLabel(vendorOption));
        optionButton.on('click.vendorOption', function (event) {
            event.preventDefault();
            event.stopPropagation();
            var button = $(this);
            var container = button.closest('.pointDiv');
            var currentIndex = container.data('index');
            var value = button.attr('data-vendor-id');
            if (!value) {
                clearVendorSelection(currentIndex);
                return;
            }
            var vendor = vendorLookupById[value];
            if (!vendor) {
                clearVendorSelection(currentIndex);
                setVendorOpenState(container, false);
                return;
            }
            applyVendorSelection(currentIndex, vendor);
            container.data('suppressVendorFocusOpen', true);
            container.find('.pointInput').trigger('focus');
        });
        list.append($('<li>').append(optionButton));
    }

    menu.append(list);

    if (selectedId && vendorLookupById[selectedId]) {
        setVendorMenuSelection(div, selectedId);
    } else {
        setVendorMenuSelection(div, null);
    }
    ...
    menu.on('keydown.vendorMenu', function (event) {
        var target = $(event.target);
        if (!target.hasClass('pointVendorOption')) {
            return;
        }
        if (event.key === 'ArrowDown' || event.key === 'Down') {
            event.preventDefault();
            var next = target.closest('li').next().find('.pointVendorOption');
            if (next.length) {
                next.trigger('focus');
            }
        } else if (event.key === 'ArrowUp' || event.key === 'Up') {
            event.preventDefault();
            var prev = target.closest('li').prev().find('.pointVendorOption');
            if (prev.length) {
                prev.trigger('focus');
            } else {
                target.closest('.pointDiv').find('.pointInput').trigger('focus');
            }
            ...
            event.preventDefault();
            var container = target.closest('.pointDiv');
            setVendorOpenState(container, false);
            container.find('.pointInput').trigger('focus');
        }
    });
...
function refreshAllVendorSelects() {
    $('#locationpoints > div.pointDiv').each(function (index, element) {
        setupVendorSelect($(element), index);
    });
}
...
function fetchVendorLocations() {
    ...
    return fetch(vendorStateUrl, {credentials: 'include'})
        ...
        .catch(function (error) {
            vendorLoading = false;
            vendorLoadErrorMessage = 'Unable to load saved locations';
...
$(document).ready(function (e) {
    ...
    $(document).off('mousedown.vendorMenu').on('mousedown.vendorMenu', function (event) {
        var target = $(event.target);
        if (target.closest('.pointDiv').length) {
            return;
        }
        closeOtherVendorMenus(null);
    });
...
function checkInput() {
    ...
    for (var i = 0; i < len; i++) {
        ...
        setupVendorSelect(div, i);
        var toggleButton = div.find('.pointVendorToggle');
        if (toggleButton.length) {
            toggleButton.off('click.vendorToggle').on('click.vendorToggle', function (event) {
                event.preventDefault();
                event.stopPropagation();
                var container = $(this).closest('.pointDiv');
                var shouldOpen = !container.hasClass('pointVendorOpen');
                if (shouldOpen) {
                    closeOtherVendorMenus(container);
                }
                setVendorOpenState(container, shouldOpen);
                if (shouldOpen) {
                    setTimeout(function () {
                        focusVendorMenu(container);
                    }, 0);
                }
            });
            toggleButton.off('keydown.vendorToggle').on('keydown.vendorToggle', function (event) {
        ...
        input.off('input.vendor').on('input.vendor', function () {
            var container = $(this).closest('.pointDiv');
            var idx = container.data('index');
            var pointForInput = ghRequest.route.getIndex(idx);
            if (pointForInput) {
                delete pointForInput.vendorId;
            }
            setVendorMenuSelection(container, null);
            setVendorOpenState(container, false);
            $(this).removeAttr('title');
        });
        input.off('focus.vendorMenu').on('focus.vendorMenu', function () {
            var container = $(this).closest('.pointDiv');
            if (container.data('suppressVendorFocusOpen')) {
                container.removeData('suppressVendorFocusOpen');
                return;
            }
            closeOtherVendorMenus(container);
            setVendorOpenState(container, true);
            setTimeout(function () {
                focusVendorMenu(container);
            }, 0);
        });
        input.off('keydown.vendorMenu').on('keydown.vendorMenu', function (event) {
